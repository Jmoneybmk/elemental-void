...
const EV = (() => {
  ...
  function load() { try { const r=localStorage.getItem(SAVE_KEY); if(!r){showNotification('No save found.','warning');return false;} state=JSON.parse(r); showNotification('Save loaded.','success'); return true; } catch(e){showNotification('Load failed.','error');return false;} }
  ...
  function startBattle(config) {
    ...
    const fb=document.getElementById('flee-btn');if(fb)fb.disabled=!canFlee;
  ...
  function setBattleButtons(disabled){
    document.querySelectorAll('.battle-btn').forEach(b=>b.disabled=disabled);
  }
  ...
  function endBattle(result){
    const ui=document.getElementById('battle-ui');if(ui)ui.classList.remove('active');
    renderStats();
    if(result==='win'&&battleState.onWin)battleState.onWin();
    else if(result==='lose'&&battleState.onLose)battleState.onLose();
    else if(result==='flee'&&battleState.onWin)battleState.onWin('fled');
  }
